...
const recover_dead_steps_1 = require("../recover-dead-steps");
...
async function writeEnvelope(db, envelope, workerId = "worker_a") {
    await db
...
async function readTraces(db, envelopeId) {
    const s = await db
...
async function readQueue(db, envelopeId) {
    const s = await db
        .collection(constants_1.COLLECTIONS.EXECUTION_QUEUE)
        .doc(envelopeId)
        .get();
...
function buildStandardEnvelope(agents, orgId = "org_aceplace") {
...
// ═══════════════════════════════════════════════════════════════════════════
(0, vitest_1.describe)("TEST RUN 1 — Successful Execution", () => {
    (0, vitest_1.it)("drives the full 5-step pipeline from created → completed with trace evidence", async () => {
        ...
        // ── Build envelope ───────────────────────────────────────────────────────
        const { envelope, steps, contexts } = buildStandardEnvelope({ coo, researcher, worker, grader }, ORG);
        ...
        for (const s of steps) {
            console.log(`    [${s.step_type.padEnd(16)}] ${s.step_id}  agent=${s.assigned_agent_id}`);
        }
        // ── State machine: created → planned → leased → executing ───────────────
        await (0, state_machine_1.transition)(ENV_ID, "planned");
        console.log("  [WORKER] plan   → status: planned");
        await (0, state_machine_1.transition)(ENV_ID, "leased");
        console.log("\n  [WORKER] claim  → status: leased");
        await (0, state_machine_1.transition)(ENV_ID, "executing");
        ...
        for (let round = 0; round <= steps.length; round++) {
            ...
            if (!env)
                throw new Error("Envelope disappeared");
            const runnable = (0, parallel_runner_1.getRunnableSteps)(env);
            ...
            for (const step of runnable) {
                ...
                await (0, parallel_runner_1.claimEnvelopeStep)({
                    envelope_id: ENV_ID,
                    step_id: step.step_id,
                    instance_id: WORKER,
                });
                // Write STEP_STARTED trace with a guaranteed-unique UUID key
                const startTraceId = `trace_step_started_${(0, crypto_1.randomUUID)()}`;
                await db.collection(constants_1.COLLECTIONS.EXECUTION_TRACES).doc(startTraceId).set({
                ...
                await (0, parallel_runner_1.finalizeEnvelopeStep)({
                    envelope_id: ENV_ID,
                    step_id: step.step_id,
                    status: "completed",
                ...
                const completeTraceId = `trace_step_completed_${(0, crypto_1.randomUUID)()}`;
                await db.collection(constants_1.COLLECTIONS.EXECUTION_TRACES).doc(completeTraceId).set({
        ...
        await (0, state_machine_1.transition)(ENV_ID, "completed");
        ...
        for (const s of finalEnv?.steps ?? []) {
            console.log(`      • [${s.step_type.padEnd(16)}] ${s.step_id}  status=${s.status}`);
        }
        ...
        for (const t of traces) {
            console.log(`    [${t.event_type.padEnd(36)}] step=${t.step_id}  agent=${t.agent_id}`);
        }
        ...
        // ── Assertions ─────────────────────────────────────────────────────────
        (0, vitest_1.expect)(finalEnv?.status).toBe("completed");
        // All 5 steps driven through the pipeline
        (0, vitest_1.expect)(executedSteps).toHaveLength(steps.length);
        // Every embedded step must be completed
        for (const s of finalEnv?.steps ?? []) {
            (0, vitest_1.expect)(s.status, `Step ${s.step_id} must be completed`).toBe("completed");
        }
        ...
        (0, vitest_1.expect)(started).toHaveLength(steps.length);
        (0, vitest_1.expect)(doneEvt).toHaveLength(steps.length);
        // State machine wrote a STATUS_TRANSITION_COMPLETED trace in Firestore
        (0, vitest_1.expect)(traces.some((t) => t.event_type === "STATUS_TRANSITION_COMPLETED")).toBe(true);
        // All trace IDs are unique — gapless chain, no collisions
        const traceIds = stepTraces.map((t) => t.trace_id);
        (0, vitest_1.expect)(new Set(traceIds).size).toBe(traceIds.length);
        console.log("\n  Result: PASS ✓");
    });
});
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
(0, vitest_1.describe)("TEST RUN 2 — Failure + Retry", () => {
    (0, vitest_1.it)("shows retry_count progression 0→1→2 then permanent failure on produce_artifact step", async () => {
        const ORG = "org_aceplace";
        ...
        await seedAgents(db, [coo, researcher, worker, grader]);
        const { envelope, steps } = buildStandardEnvelope({ coo, researcher, worker, grader }, ORG);
        await writeEnvelope(db, envelope, WORKER);
        const ENV_ID = envelope.envelope_id;
        ...
        for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
            ...
            const stepNow = envNow.steps.find((s) => s.step_id === workerStep.step_id);
            if (stepNow.status !== "ready" && stepNow.status !== "pending")
                break;
            ...
            if (canRetry) {
                ...
                console.log(`  [WORKER] → step FAILED — rescheduled (retry_count → ${nextRetry})`);
            }
            else {
                await (0, parallel_runner_1.finalizeEnvelopeStep)({
                    envelope_id: ENV_ID,
                ...
                console.log(`  [WORKER] → step FAILED permanently (retry_count → ${nextRetry}, max=${MAX_RETRIES})`);
            }
        }
        const finalEnv = await readEnvelope(db, ENV_ID);
        const failedStep = finalEnv.steps.find((s) => s.step_id === workerStep.step_id);
        ...
        (0, vitest_1.expect)(failedStep.status).toBe("failed");
        (0, vitest_1.expect)(failedStep.retry_count).toBe(MAX_RETRIES);
        console.log("\n  Result: PASS ✓");
    });
});
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
(0, vitest_1.describe)("TEST RUN 3 — Lease Conflict (Concurrency)", () => {
    (0, vitest_1.it)("rejects second worker with FORK_DETECTED while first holds the authority lease", async () => {
        const ORG = "org_aceplace";
        ...
        await seedAgents(db, [coo, researcher, worker, grader]);
        const { envelope } = buildStandardEnvelope({ coo, researcher, worker, grader }, ORG);
        ...
        const leaseA = await (0, per_agent_authority_1.acquirePerAgentLease)(ENV_ID, TARGET_AGENT, WORKER_A);
        ...
        let forkError = null;
        try {
            await (0, per_agent_authority_1.acquirePerAgentLease)(ENV_ID, TARGET_AGENT, WORKER_B);
        }
        catch (e) {
            forkError = e;
        }
        ...
        console.log(`    current_instance_id : ${lAfterFork?.current_instance_id}  (still Worker A)`);
        console.log(`    status              : ${lAfterFork?.status}`);
        console.log("\n  Firestore — execution_queue snapshot:");
        console.log(`    claimed_by : ${queue?.claimed_by}  (only Worker A is authorized)`);
        console.log("\n  Proof: no duplicate execution — Worker B rejected before any step was claimed");
        (0, vitest_1.expect)(forkError).not.toBeNull();
        (0, vitest_1.expect)(forkError?.message).toBe("FORK_DETECTED");
        (0, vitest_1.expect)(lAfterFork?.current_instance_id).toBe(WORKER_A);
        (0, vitest_1.expect)(queue?.claimed_by).toBe(WORKER_A);
        console.log("\n  Result: PASS ✓");
    });
});
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
(0, vitest_1.describe)("TEST RUN 4 — Identity Failure", () => {
    (0, vitest_1.it)("halts execution and quarantines envelope on IDENTITY_FINGERPRINT_MISMATCH", async () => {
        const ORG = "org_aceplace";
        // Seed a real agent with a real fingerprint
        const coo = makeAgent("agent_coo_004", "COO", ORG);
        await db.collection(constants_1.COLLECTIONS.AGENTS).doc(coo.agent_id).set(coo);
        // Envelope carries a TAMPERED fingerprint
        const TAMPERED_FP = "deadbeef00000000000000000000000000000000000000000000000000000001";
        ...
        const result = await (0, identity_1.verifyIdentityForAgent)(envelopeId, tamperedEnvelope, coo.agent_id);
        ...
        for (const t of traces) {
            console.log(`    [${t.event_type.padEnd(36)}] agent=${t.agent_id}`);
        }
        ...
        (0, vitest_1.expect)(traces.some((t) => t.event_type === "IDENTITY_FINGERPRINT_MISMATCH")).toBe(true);
        console.log("\n  Result: PASS ✓");
    });
});
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
(0, vitest_1.describe)("TEST RUN 5 — Restart / Resurrection", () => {
    (0, vitest_1.it)("recovers a stale executing step to ready — no duplicate execution, correct resume point", async () => {
        const ORG = "org_aceplace";
        ...
        const modifiedSteps = steps.map((s) => {
        ...
        console.log(`  Completed steps : ${planStep.step_id}, ${assignStep.step_id}`);
        ...
        await (0, recover_dead_steps_1.recoverEnvelopeDeadSteps)(envelope.envelope_id, 0);
        ...
        console.log(`    ${planStep.step_id}  : status=${rPlan.status}   ← untouched (was already completed)`);
        console.log(`    ${assignStep.step_id}: status=${rAssign.status}   ← untouched (was already completed)`);
        console.log(`    ${crashStep.step_id} : status=${rCrash.status}  ← RECOVERED`);
        ...
        console.log(`    ${evalStep.step_id}  : status=${rEval.status}    ← still pending upstream`);
        console.log("\n  Proof: resumed from produce_artifact — NOT from beginning. " +
            "plan and assign remain completed.");
        (0, vitest_1.expect)(rCrash.status).toBe("ready");
        (0, vitest_1.expect)(rCrash.retry_count).toBe(1);
        (0, vitest_1.expect)(rCrash.claimed_by_instance_id).toBeNull();
        (0, vitest_1.expect)(rCrash.claimed_at).toBeNull();
        // Previously completed steps must remain untouched
        (0, vitest_1.expect)(rPlan.status).toBe("completed");
        (0, vitest_1.expect)(rAssign.status).toBe("completed");
        // Downstream steps remain pending (no skipping)
        (0, vitest_1.expect)(rEval.status).toBe("pending");
        console.log("\n  Result: PASS ✓");
    });
});
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
(0, vitest_1.describe)("TEST RUN 6 — #us# Protocol Enforcement", () => {
    (0, vitest_1.it)("rejects invalid message types and proves messages are never executed without valid routing", async () => {
        const ORG = "org_aceplace";
        ...
        await db.collection(constants_1.COLLECTIONS.EXECUTION_ENVELOPES).doc(envelopeId).set({
            envelope_id: envelopeId,
            org_id: ORG,
            status: "executing",
            steps: [],
        ...
        const invalidMsg = (0, us_message_engine_1.createUSMessage)({
            message_type: "HACK::BYPASS::EXECUTE",
        ...
        let unknownTypeErr = null;
        try {
            await (0, us_message_engine_1.handleUSMessage)(invalidMsg);
        }
        catch (e) {
            unknownTypeErr = e;
        }
        console.log(`      REJECTED with : ${unknownTypeErr?.message}`);
        console.log("      Message was NOT executed.");
        (0, vitest_1.expect)(unknownTypeErr).not.toBeNull();
        (0, vitest_1.expect)(unknownTypeErr?.message).toBe("UNKNOWN_MESSAGE_TYPE");
        // ── Scenario B: Valid #us#.task.plan — handled safely, returns null ──────
        console.log("\n  [B] Scenario: valid #us#.task.plan message");
        const planMsg = (0, us_message_engine_1.createUSMessage)({
        ...
        (0, vitest_1.expect)(constructed.protocol).toBe("#us#");
        (0, vitest_1.expect)(constructed.version).toBe("1.0");
        (0, vitest_1.expect)(constructed.metadata?.timestamp).toBeDefined();
        ...
        const storedMsg = (0, us_message_engine_1.createUSMessage)({
            message_type: "#us#.task.plan",
        ...
        const msgId = await (0, us_message_engine_1.storeUSMessage)(storedMsg);
        console.log(`      message_id stored: ${msgId}`);
        const msgSnap = await db
            .collection(constants_1.COLLECTIONS.EXECUTION_MESSAGES)
            .doc(msgId)
            .get();
        console.log(`      identity_fingerprint in message: ${msgSnap.data()?.identity_fingerprint}`);
        const traces = await readTraces(db, envelopeId);
        console.log(`      execution_traces entries: ${traces.length}`);
        (0, vitest_1.expect)(msgSnap.exists).toBe(true);
        (0, vitest_1.expect)(msgSnap.data()?.identity_fingerprint).toBe(fp);
        (0, vitest_1.expect)(traces.length).toBeGreaterThan(0);
        // ── Scenario E: Missing envelope → rejected before any handler runs ───────
        console.log("\n  [E] Scenario: message referencing non-existent envelope");
        const orphanMsg = (0, us_message_engine_1.createUSMessage)({
            message_type: "#us#.task.plan",
        ...
        let orphanErr = null;
        try {
            await (0, us_message_engine_1.handleUSMessage)(orphanMsg);
        }
        catch (e) {
            orphanErr = e;
        }
        console.log(`      REJECTED with: ${orphanErr?.message}`);
        (0, vitest_1.expect)(orphanErr?.message).toBe("ENVELOPE_NOT_FOUND");
        console.log("\n  Result: PASS ✓");
    });
});
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
(0, vitest_1.describe)("TEST RUN 7 — Determinism Proof", () => {
    (0, vitest_1.it)("produces identical step sequences + fingerprints for two independent envelope builds", async () => {
        const ORG = "org_aceplace";
        const agents = {
        ...
        console.log("═".repeat(72));
        // ── Build A ──────────────────────────────────────────────────────────────
        const { envelope: envA, steps: stepsA, contexts: ctxA } = buildStandardEnvelope(agents, ORG);
        await writeEnvelope(db, envA, "worker_a_07");
        // ── Build B ──────────────────────────────────────────────────────────────
        const { envelope: envB, steps: stepsB, contexts: ctxB } = buildStandardEnvelope(agents, ORG);
        ...
        // ── Assertions ────────────────────────────────────────────────────────────
        (0, vitest_1.expect)(stepsA).toHaveLength(stepsB.length);
        for (let i = 0; i < stepsA.length; i++) {
            (0, vitest_1.expect)(stepsA[i].step_type).toBe(stepsB[i].step_type);
            (0, vitest_1.expect)(stepsA[i].role).toBe(stepsB[i].role);
            (0, vitest_1.expect)(stepsA[i].assigned_agent_id).toBe(stepsB[i].assigned_agent_id);
            (0, vitest_1.expect)(stepsA[i].depends_on?.length ?? 0).toBe(stepsB[i].depends_on?.length ?? 0);
        }
        (0, vitest_1.expect)(fpA).toBe(fpB);
        (0, vitest_1.expect)(allTypesMatch).toBe(true);
        (0, vitest_1.expect)(allDepsMatch).toBe(true);
        // Verify no extra steps appear in either run
        const extraInA = stepsA.filter((sA) => !stepsB.some((sB) => sB.step_type === sA.step_type));
        const extraInB = stepsB.filter((sB) => !stepsA.some((sA) => sA.step_type === sB.step_type));
        (0, vitest_1.expect)(extraInA).toHaveLength(0);
        (0, vitest_1.expect)(extraInB).toHaveLength(0);
        console.log(`  No extra steps in A or B: ✓`);
        console.log("\n  Result: PASS ✓");
    });
});
//# sourceMappingURL=phase2-validation.test.js.map