/**
 * NOVA ACE → #us#.task.handoff → execution envelope (entry contract).
 */
...
export type NovaHandoffMessage = {
  protocol: "#us#";
  version?: "1.0";
...
export type CreateNovaHandoffInput = {
  org_id: string;
  requested_by_user_id: string;
  session_id: string;
...
export function createNovaHandoff(input: CreateNovaHandoffInput): NovaHandoffMessage {
  ...
  return {
    ...
    identity: {
      agent_id: "nova_ace",
      identity_fingerprint: "nova_ace_interface",
    },
    authority: {
...
export function validateNovaHandoff(handoff: unknown): asserts handoff is NovaHandoffMessage {
...
async function resolveAgentFingerprint(agentId: string): Promise<string> {
  ...
  const doc = await getDb().collection(COLLECTIONS.AGENTS).doc(agentId).get();
  if (doc.exists) {
    const agent = doc.data() as AgentIdentity;
    return computeFingerprint(agent.canonical_identity_json);
  }
...
export async function acceptNovaHandoff(handoff: NovaHandoffMessage): Promise<{
  success: boolean;
  envelope_id: string;
}> {
  validateNovaHandoff(handoff);
  ...
  runEnvelopeParallel({
    ...
    console.error("[NOVA_HANDOFF] Parallel runner failed:", err);
  });
