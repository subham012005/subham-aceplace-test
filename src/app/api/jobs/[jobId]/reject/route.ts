...
export async function POST(
    ...
    try {
        ...
        const response = await fetch(n8nUrl, {
            ...
            body: JSON.stringify({ job_id: jobId, reason: reason || "Operator Rejection" }),
        });

        if (!response.ok) {
            const error = await response.text();
            return NextResponse.json({ error: "n8n rejection failed", details: error }, { status: response.status });
        }

        // 3. Update Firestore Status
        await jobRef.update({
            status: "rejected",
            failure_reason: reason || "Operator Rejection",
        ...
        await adminDb.collection("job_traces").add({
            ...
            message: `Operator has rejected the manifest: ${reason || "No reason provided"}`,
        ...
        return NextResponse.json({ error: error.message || "Failed to reject job" }, { status: 500 });
    }
}
