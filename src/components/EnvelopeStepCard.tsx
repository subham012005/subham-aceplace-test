...
import { STEP_TYPE_CONFIG, STEP_STATUS_DISPLAY } from "@aceplace/runtime-core/shared";
import type { EnvelopeStep, StepType } from "@aceplace/runtime-core/shared";
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span className={cn("text-[10px] font-black uppercase tracking-widest", statusDisplay?.color || "text-slate-500")}>
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <span className="text-[10px] font-mono text-slate-700 flex items-center gap-0.5">
        ...
        <div className="border-t border-white/5 p-4 space-y-3 animate-in slide-in-from-top-2 duration-200">
          ...
          <div className="grid grid-cols-2 gap-3">
            {(step as any).started_at && (
              <div className="space-y-0.5">
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-600">Started</span>
                <p className="text-[11px] font-mono text-slate-400">{new Date((step as any).started_at).toLocaleString()}</p>
              </div>
            )}
            {(step as any).completed_at && (
              <div className="space-y-0.5">
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-600">Completed</span>
                <p className="text-[11px] font-mono text-slate-400">{new Date((step as any).completed_at).toLocaleString()}</p>
              </div>
            )}
          </div>
          ...
          {(step as any).input && Object.keys((step as any).input).length > 0 && (
            <div className="space-y-1">
              <span className="text-[10px] font-black uppercase tracking-widest text-cyan-500/50">Input Data</span>
          ...
          {((step as any).output || step.output_ref) && (
            <div className="space-y-1">
              <span className="text-[10px] font-black uppercase tracking-widest text-emerald-500/50">Output Intelligence</span>
              <pre className="text-[11px] font-mono text-emerald-400/90 bg-black/40 border border-emerald-500/10 p-3 max-h-48 overflow-auto whitespace-pre-wrap">
              {(() => {
          ...
          {(step as any).error && (
            <div className="space-y-1">
              <span className="text-[10px] font-black uppercase tracking-widest text-red-500/50">Error</span>
              <p className="text-[11px] font-mono text-red-400/80 bg-red-500/5 border border-red-500/20 p-3">
              {(step as any).error}
              </p>
            </div>
          )}
